...
from sdot import generate_tensor_from_seeds_vmap
from sit import SiT_models
from loss import SILoss

from dataset import LMDBLatentsDataset
from diffusers.models.autoencoders.vae import DiagonalGaussianDistribution
from imagenet_sdot_utils.sdot_latent_dataset import SdotRebalanceDatasetSeed
...
def main(args):    
    ...
    if args.sdot:
        train_dataset=SdotRebalanceDatasetSeed(total_samples_needed=args.epochs*1281167)
    else:
        train_dataset = LMDBLatentsDataset(args.data_dir, flip_prob=0.5)
    ...
    steps_per_epoch = 1281167//local_batch_size // accelerator.gradient_accumulation_steps
    ...
    for epoch in range(args.epochs):
        ...
        for batch in train_dataloader:
            if args.sdot:
                ...
                keys=jax.dlpack.from_dlpack(torch.utils.dlpack.to_dlpack(keys))
                noises=generate_tensor_from_seeds_vmap(keys, x.shape[1:])
                noises=torch.from_dlpack(jax.dlpack.to_dlpack(noises))
                noises=noises.float().to(device, non_blocking=True)

                # noises = noises.to(device, non_blocking=True).float()
            ...
            with accelerator.accumulate(model):
                ...
                loss, loss_ref = loss_fn(model, x, noises, model_kwargs)
                loss_mean = loss.mean()
            ...
            logs = {
                ...
                "grad_norm": 0 # when using grad_accumulation please switch grad_norm to 0 since otherwise it throws a bug
            }
...
def parse_args(input_args=None):
    ...
    # dataset
    parser.add_argument("--data-dir", type=str, default="/home/ubuntu/code/MeanFlow/data/ImageNet/train_vae_latents_lmdb")
    ...
    parser.add_argument("--ratio-r-not-equal-t", type=float, default=0.25, help="Ratio of samples where r≠t")
    ...
    parser.add_argument('--no-sdot', dest='sdot', action='store_false')
